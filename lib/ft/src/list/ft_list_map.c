...
/*   Updated: 2020/07/31 07:51:36 by abaisago         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list_link	*ft_list_map(t_list *list,
				t_list_link *(*f)(t_list_link*, void*), void *input,
				void (*del)(void*, size_t))
{
	...
	if (list->len == 0 || f == NULL)
		return (NULL);
	if ((begin = (*f)(list->head, input)) == NULL)
		return (NULL);
	link = list->head;
	map_link = begin;
	while (link->next != list->head)
	{
		if ((map_link->next = (*f)(link->next, input)) == NULL)
		{
